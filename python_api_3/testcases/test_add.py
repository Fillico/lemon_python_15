...
class AddTest(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.logs = do_log.DoLogs(do_log.log_file, 'add')
        cls.logs.info("准备测试前置")
        cls.http_request = HttpSessionRequest()
        cls.mysql = do_mysql.DoMysql()

    @data(*cases)
    def test_add(self, case):
        self.logs.info("开始执行测试：{}".format(case.title))
        # 在请求之前替换参数化的值
        ...
        try:
            ...
            self.logs.error('case:{},请求的数据:{},测试报错：{}'.format(case.title, case.data, e))
            raise e
        self.logs.info("结束测试：{}".format(case.title))

    @classmethod
    def tearDownClass(cls):
        cls.http_request.session_close()
        cls.mysql.close()
        cls.logs.remove_handler()
